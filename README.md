...
- arXiv references use `@ARTICLE` in this project, with `journal={arXiv preprint:...}`. Include the base official arXiv identifier, such as `arXiv:2403.00561`, without version number or subject class unless the target venue asks for them; use `url` or `note` when needed so the identifier appears in the rendered reference.
...
- 本项目中 arXiv 引用使用 `@ARTICLE`，并写成 `journal={arXiv preprint:...}`。引用应包含基础官方 arXiv identifier，例如 `arXiv:2403.00561`；默认不带版本号和学科分类，除非目标 venue 要求；必要时使用 `url` 或 `note`，确保最终参考文献中能显示该 identifier。
