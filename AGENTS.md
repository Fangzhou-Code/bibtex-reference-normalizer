...
引用预印本前，先检查是否已有正式发表或同行评议版本。本项目中 arXiv 条目使用 `@ARTICLE`，并写成 `journal={arXiv preprint:...}`。最终打印出的参考文献必须包含基础官方 arXiv identifier，例如 `arXiv:2403.00561`，旧格式如 `arXiv:hep-th/9901001`；默认不带版本号和学科分类，除非目标 venue 明确要求。identifier 中不要插入空格或额外标点。除非目标 venue 明确要求，否则 arXiv 条目不添加 `note`、`url` 或 `doi`。
